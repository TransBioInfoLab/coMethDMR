...
BiocManager::install("TransBioInfoLab/MethReg")
```
...
betasChr22_df <- betasChr22_df[rownames(betasChr22_df) %in% unlist(Cgi_ls)[1:20],]
```
...
gene3_200 <- CloseBySingleRegion(
  CpGs_char = geneList$ARFGAP3, arrayType = "450k", maxGap = 200, minCpGs = 3
)
CpGsOrdered_df <- lapply(gene3_200, OrderCpGsByLocation, arrayType = "450k", output = "dataframe")
names(gene3_200) <- lapply(CpGsOrdered_df, NameRegion)
